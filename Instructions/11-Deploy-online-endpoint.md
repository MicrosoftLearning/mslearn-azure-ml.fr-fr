---
lab:
  title: Déployer un modèle sur un point de terminaison en ligne managé
---

# Déployer un modèle sur un point de terminaison en ligne managé

Pour consommer un modèle dans une application et obtenir des prédictions en temps réel, vous voulez déployer le modèle sur un point de terminaison en ligne managé. Un modèle MLflow est facilement déployé puisque vous n’avez pas besoin de définir l’environnement ni de créer le script de scoring.

Dans cet exercice, vous allez déployer un modèle MLflow sur un point de terminaison en ligne managé et le tester sur un échantillon de données. 
...
1. Ouvrez le notebook **Labs/11/Deploy to online endpoint.ipynb**.
