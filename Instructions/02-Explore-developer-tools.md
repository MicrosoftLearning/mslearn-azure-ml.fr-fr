...
1. Une fois le travail effectué, vous pouvez trouver l’*AUC* et la *justesse* du modèle sous **Métriques**. 
1. Sous **Sorties + journaux**, vous trouverez :
    - Le fichier pickle de modèle dans le dossier **outputs**. 
    - La sortie du script dans **user_logs/std_log.txt**. La sortie des instructions **print** dans le script s’y trouve. En cas d’erreur due à un problème avec votre script, vous y trouverez également le message d’erreur.
