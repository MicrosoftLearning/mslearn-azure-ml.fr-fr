---
lab:
  title: Créer et explorer le tableau de bord de l’IA responsable
---

# Créer et explorer le tableau de bord de l’IA responsable

Après avoir entraîné votre modèle, vous pouvez l’évaluer pour déterminer s’il fonctionne comme prévu. Outre les métriques de performances, vous pouvez prendre en compte d’autres facteurs. Le tableau de bord de l’IA responsable dans Azure Machine Learning vous permet d’analyser les données et les prédictions du modèle pour identifier tout biais ou partialité.

Dans cet exercice, vous allez préparer vos données et créer un tableau de bord d’IA responsable dans Azure Machine Learning.
...
## Créer un pipeline pour évaluer des modèles et les envoyer depuis un notebook

Maintenant que vous disposez de toutes les ressources nécessaires, vous pouvez exécuter le notebook pour récupérer les composants intégrés de l’IA responsable, créer un pipeline et envoyer le pipeline pour générer un tableau de bord de l’IA responsable.

1. Ouvrez le notebook **Labs/10/Create Responsible AI dashboard.ipynb**.
